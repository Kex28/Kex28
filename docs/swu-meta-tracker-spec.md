...
- **Card images.** Pull images from swu-db.com (or swuapi.com's card `frontImageUrl`) server-side and store/serve them from Supabase Storage, so cards render with art instead of text everywhere they appear (decklists, card trends, My Decks) — matching the spec's "nothing links out" rule.
- **Real SWUDB URL import.** My Decks currently accepts SWUDB's JSON export pasted by hand. Importing directly from a SWUDB deck URL needs a small server-side function (browsers block the cross-origin fetch directly) — a good fit for a Vercel serverless function once the site is deployed.
- **Global filters everywhere.** Date range, tier, and aspect filters exist on Tournaments and Archetypes; extending the same filter bar to Meta Trends, Counter Meta, and Cards would make the spec's "filters everything" line fully true rather than partially true.
